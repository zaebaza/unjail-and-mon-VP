...
COIN=$(${BINARY} q staking params -o j | jq -r '.bond_denom')
...
if [ "$ANSWER" == "yes" ]; then
    ...
    if [[ $VPOWER == 0 ]]; then
        ...
        echo "-------------------------------------------------------------------"
        echo $PASS | ${BINARY} tx slashing unjail --fees ${FEE} --chain-id=${CHAIN} --from ${KEY_NAME} --node http://localhost:${RPC_PORT} -y | grep "raw_log\|txhash"
        sleep 15s
